...
function onSelectOption(self) {
    ...
    for (i = 0; i < allParameters.length; i++) {
        ...
        var isRadioactiveParam = parameterDiv.querySelector("script[src='/plugin/radioactive-parameter-plugin/selectOption.js']") !== null;
            ...
            if (enabledParametersSet.has(parameter.textContent.trim())) {
                ...
                if (hideMode === "Hide elements") {
                    ...
                    if (isRadioactiveParam) {
                        var childElem = parameterDiv.querySelector("select[name='selectedOption']");
